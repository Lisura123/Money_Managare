...
// ─── Create Card Entry Modal ──────────────────────────────────────────────
function CreateCardEntryModal({ open, showrooms, cardAccounts, onClose, onSaved }) {
  ...
  const availableAccounts = showroomId
    ? cardAccounts.filter(c => String(c.showroom_id) === String(showroomId) && (c.is_active ?? true))
    : []

  const reset = () => {
    setShowroomId(''); setCardAccountId(''); setEntryDate(todayString())
    setAmount(''); setNotes('')
  }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!showroomId) { toast.error('Please select a showroom.'); return }
    if (!cardAccountId) { toast.error('Please select a bank account.'); return }
    ...
    try {
      await api.post('/card-entries', {
        card_account_id: Number(cardAccountId),
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-navy rounded-xl shadow-2xl p-6 w-full max-w-sm animate-fade-in">
        <h3 className="font-heading font-semibold text-navy dark:text-white text-base mb-4">New Bank Entry</h3>
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            <label className="form-label">Showroom *</label>
            <select className="form-input" value={showroomId} onChange={e => { setShowroomId(e.target.value); setCardAccountId('') }} required>
          ...
          <div>
            ...
            <select className="form-input" value={cardAccountId} onChange={e => setCardAccountId(e.target.value)} disabled={!showroomId} required>
              <option value="">{showroomId ? 'Select an account…' : 'Select a showroom first'}</option>
              {availableAccounts.map(c => (
                <option key={c.id} value={c.id}>{c.bank_name} ····{c.last_four}</option>
              ))}
            </select>
            {showroomId && availableAccounts.length === 0 && (
              <p className="form-error">No active bank accounts for this showroom.</p>
            )}
          </div>
          <div>
...
function EditCardEntryModal({ open, entry, onClose, onSaved }) {
  ...
  const currentVal = parseFloat(entry?.amount || 0)
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      await api.put(`/card-entries/${entry.id}`, {
...
export default function CardEntriesAdminPage() {
  ...
  const { data: cardAccountsData } = useFetch(ENDPOINTS.CARD_ACCOUNTS)
  const showrooms = Array.isArray(showroomsData) ? showroomsData : (showroomsData?.data || [])
  const cardAccounts = Array.isArray(cardAccountsData) ? cardAccountsData : (cardAccountsData?.data || [])

  // Bank balance for selected showroom (sum of active card accounts)
  const selectedShowroom = showrooms.find(s => String(s.id) === String(showroomId))
  const showroomBankBalance = showroomId
    ? cardAccounts.filter(c => String(c.showroom_id) === String(showroomId) && (c.is_active ?? true))
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      {/* Showroom bank balance banner */}
      {showroomBankBalance !== null && (
        <div className="flex items-center justify-between px-4 py-3 rounded-xl bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800">
          <div className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400">
            <MdFilterList className="w-4 h-4" />
            <span className="text-sm font-medium">{selectedShowroom?.name} — Bank Balance</span>
          </div>
          <span className={`text-base font-bold ${showroomBankBalance >= 0 ? 'text-success' : 'text-error'}`}>
            {formatCurrency(showroomBankBalance)}
          </span>
        </div>
      )}
            ...
            return (
              <div key={entry.id} className="card flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0 space-y-1">
                  ...
                  {/* Showroom + bank account */}
                  <p className="text-xs text-gray-400 dark:text-gray-500">{entry.showroom?.name}</p>
                  <p className="text-xs text-gray-400 dark:text-gray-500 font-mono">{cardDisplay}</p>
                  {/* Notes */}
                  {entry.notes && (
