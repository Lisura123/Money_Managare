...
class _CashAdjustmentScreenState extends State<CashAdjustmentScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<CashEntryProvider>().fetchAdjustments();
  ...
  Future<void> _confirmDeleteAdjustment(dynamic adjustment) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('Delete Cash Adjustment'),
        content: Text(
            'Are you sure you want to delete this adjustment of ${Formatters.currency(adjustment.adjustedAmount)}?'),
        actions: [
          TextButton(
    ...
    try {
      await context.read<CashEntryProvider>().deleteAdjustment(adjustment.id);
      if (mounted) {
  ...
  void _selectAll() {
    final adjustments = context.read<CashEntryProvider>().adjustments;
    setState(() {
  ...
  Future<void> _bulkDelete() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
            'Are you sure you want to delete $count ${count == 1 ? 'adjustment' : 'adjustments'}?'),
        actions: [
          TextButton(
              onPressed: () => Navigator.pop(ctx, false),
              child: const Text('Cancel')),
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      await context.read<CashEntryProvider>().addAdjustment(
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text('Cash adjustment added'),
        backgroundColor: AppColors.success,
      ));
      _amountController.clear();
      _reasonController.clear();
      setState(() => _selectedShowroom = null);
      context.read<CashEntryProvider>().fetchAdjustments();
    ...
    return GestureDetector(
      ...
      child: Scaffold(
        appBar: _isSelectionMode
            ...
            : AppBar(title: const Text('Cash Adjustments')),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        DropdownButtonFormField<Showroom>(
                          ...
                          isExpanded: true,
                          hint: const Text('Showroom (optional)'),
                          ...
                          items: [
                            ...
                            ...showrooms.map((s) => DropdownMenuItem(
                                value: s, child: Text(s.name))),
                          ],
                          onChanged: (v) =>
                              setState(() => _selectedShowroom = v),
                        ),
                        if (_selectedShowroom != null)
                          Builder(builder: (context) {
                            final dash = context.watch<DashboardProvider>();
                            final snap = dash.summary?.today.perShowroom
                                .where((s) =>
                                    s.showroomId == _selectedShowroom!.id)
                                .firstOrNull;
                            if (snap == null) return const SizedBox.shrink();
                            final total =
                                snap.cashMainAdjusted + snap.cashManoAdjusted;
                            return Padding(
                                  ...
                                  children: [
                                    ...
                                    Row(
                                      children: [
                                        ...
                                        Text(
                                          'Main: ${Formatters.currency(snap.cashMainAdjusted)}  ·  Mano: ${Formatters.currency(snap.cashManoAdjusted)}',
                                          style: GoogleFonts.inter(
                                              fontSize: 11,
