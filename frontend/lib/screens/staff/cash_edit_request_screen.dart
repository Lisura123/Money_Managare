...
class CashEditRequestScreen extends StatefulWidget {
  final DailyCashEntry entry;
  const CashEditRequestScreen({super.key, required this.entry});

  @override
  State<CashEditRequestScreen> createState() => _CashEditRequestScreenState();
}

class _CashEditRequestScreenState extends State<CashEditRequestScreen> {
  ...
  void initState() {
    super.initState();
    _amountController.text = widget.entry.cashAmount.toStringAsFixed(2);
  ...
  Future<void> _submit() async {
    ...
    if (newAmount != null &&
        (newAmount - widget.entry.cashAmount).abs() > 0.001) {
      requestedChanges['cash_amount'] = newAmount;
    }
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Scaffold(
        appBar: AppBar(title: const Text('Request Edit — Cash Entry')),
        body: SafeArea(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _OriginalEntryCard(
                    ...
                    amount: Formatters.currency(widget.entry.cashAmount),
                  ...
                  Text(
                    'Modify the fields you want to change. Date cannot be changed.',
